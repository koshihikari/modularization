...
jQuery(document).ready(function($){
	...
	MYNAMESPACE.view.AutoAddressManagerForIE10.prototype = {
		...
		,initialize: function() {//初期設定的な
			...
			this._Num_Name1 = $('input.AutoAddressNum1').attr('name')
			this._Num_Name2 = $('input.AutoAddressNum2').attr('name')
...
/*消すべき？*/	$(thisObj).trigger('onChangeAddressName',[thisObj._Num_Name1,thisObj._Num_Name2,thisObj._Text_Name1,thisObj._Text_Name2,thisObj._Text_Name3,rimit]);
			...
			if($('input.AutoAddressNum2').length === 0){
				thisObj._AddressNumlength = 7
				thisObj._AddressNumInput = $('input.AutoAddressNum1')
			} else {
				thisObj._AddressNumlength = 4
				thisObj._AddressNumInput = $('input.AutoAddressNum2')
			//次のフォームへ移る
				$('form')
					.on('keyup', 'input.AutoAddressNum1', function(event) {
						if($('input.AutoAddressNum1').val().replace('-', '').replace('－', '').length == 3){
							$('input.AutoAddressNum2').focus();
						};
					})
			}
...
/*消すべき？*/	$(thisObj).trigger('onChangeAddressName',[thisObj._Num_Name1,thisObj._Num_Name2,thisObj._Text_Name1,thisObj._Text_Name2,thisObj._Text_Name3,rimit]);
		...
		,changeNameAddressNum: function() {
			var thisObj = this;
			if($('input.AutoAddressNum1').length !== 0){
				// thisObj._Num_Name1 = $('input.AutoAddressNum1').attr('name')
				$('input.AutoAddressNum1').attr('name','name1')
				if($('input.AutoAddressNum2').length !== 0){
					// thisObj._Num_Name2 = $('input.AutoAddressNum2').attr('name')
					$('input.AutoAddressNum2').attr('name','name2')
