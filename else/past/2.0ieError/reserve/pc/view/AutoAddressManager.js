		...
		,autoAddress: function() {
			...
			thisObj._AddressNumInput
				.on('keyup change', function(event) {
					if($(this).val().replace('-', '').replace('－', '').length == thisObj._AddressNumlength){
						// var countval = thisObj._AddressNumInput.val();
						var countval = $(this).val().replace('-', '').replace('－', '').replace(/[Ａ-Ｚａ-ｚ０-９]/g,function(s){return String.fromCharCode(s.charCodeAt(0)-0xFEE0)});
						$(this).val(countval);
