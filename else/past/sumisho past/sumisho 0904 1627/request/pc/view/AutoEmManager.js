

jQuery.noConflict();
jQuery(document).ready(function($){

	MYNAMESPACE.namespace('view.AutoEmManager');
	MYNAMESPACE.view.AutoEmManager = function() {
		this.initialize.apply(this, arguments);
	};
	MYNAMESPACE.view.AutoEmManager.prototype = {
		_instances			: {}
		,_classNum			: -1
		,_currentIndex			: -1

		,initialize: function() {//初期設定的な
			var thisObj = this;
			this._instances = {//
			}
			_.bindAll(
				this
				,'autoEm'
			);
		}
		,autoEm: function() {
			var thisObj = this;
			// var EmSelector = $('input:not(#txtCity)')
			var EmSelector = $('input:not(input[type="select"],input[type="radio"],#txtCity,#txtDistrict,#txtAddress,#txtBuilding)')
			$('form').on('blur',EmSelector,function(){
				//セレクタ機能しているかどうか
				// var thisID = $(this).attr('id')
				// switch(thisID) {
				// 	case 'txtHopeArea':
				// 	case 'txtProductName':
				// 	case 'txtAcceptOther':
				// 	case 'txtAcceptOtherInfo':
				// 		chkZenkakuCond(this,0);
				// 		break;
				// 	default:
						var replaceVal = $(this).val().replace('－','-').replace('＠','@').replace('＿','_').replace('．','.').replace(/[Ａ-Ｚａ-ｚ０-９]/g,function(s){return String.fromCharCode(s.charCodeAt(0)-0xFEE0)})
						$(this).val(replaceVal)
						// break;
				// }

				// var thisID = $(this).attr('id')
				// switch(thisID) {
				// 	case 'txtCity':
				// 	case 'txtDistrict':
				// 	case 'txtAddress':
				// 		// chkZenkakuCond(this,0);
				// 		break;
				// 	case 'txtBuilding':
				// 		// chkZenkakuCond(this,1);
				// 		break;
				// }
			});
		}
	}
});