...
/*check!!!!*/	var validation = $j("form")
		...
		,setCheckboxAction: function() {
			...
			// $('#txtAcceptOther').blur(function(){

			$('form').on('click blur', 'input.ValidationCheckboxInput,#txtAcceptOther' ,function(event){
				var thisClosestTd = $(this).closest('.ValidationCheckboxTd')
				if(thisClosestTd.find('input.ValidationCheckboxInput').filter(':checked').val() !== undefined){
						thisClosestTd.removeClass('blank');
...
//住商専用　↓↓↓　+++++++++++++++++++++++
				} else if(thisClosestTd.closest('tr').next().find('input').val() != ''){
						thisClosestTd.removeClass('blank');
//住商専用　↑↑↑　+++++++++++++++++++++++
				} else {
		...
		,firstValidate: function() {
			...
			$('.ValidationCheckboxTd').each(function(){
				var thisFindInput = $(this).find('input.ValidationCheckboxInput')
