...
jQuery(document).ready(function($){
	...
	MYNAMESPACE.view.AutoAddressManager.prototype = {
		...
		,autoAddress: function() {
			var thisObj = this;
			var Number1 = $('#'+thisObj._AutoAddressIdObj['Num1'])
			// $('form')
			// 	.on('keyup', $('#'+thisObj._AutoAddressIdObj['Num1']+',#'+$('#'+thisObj._AutoAddressIdObj['Num2'])), function(event) {
			// 		var countval = $(this).val().replace('-', '').replace('－', '').replace(/[Ａ-Ｚａ-ｚ０-９]/g);
			// 		if(countval.length === 3){
			// 			thisObj._AddressNumInput.focus($(this).next('input,select'));
			// 		}
			// })
...
// 			$('form')
// 				.on('keyup', thisObj._AddressNumInput3, function(event) {
// 				var countval3 = thisObj._AddressNumInput3.val().replace('-', '').replace('－', '').replace(/[Ａ-Ｚａ-ｚ０-９]/g);
// 				var count3 = countval3.length;
// 						if(count3 == 3){
// 							thisObj._AddressNumInput3.val(countval3)
