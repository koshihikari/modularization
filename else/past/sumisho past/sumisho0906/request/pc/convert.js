...
//ID付与(nameが被った場合、単にiを付与する方が処理が軽い)
		var inputSelectSelector = $('input,select').not('input[type="radio"],input[name="col_34"],select[name="col_33"],select[name="col_41"],select[name="col_42"]')
		inputSelectSelector.each(function(i) {//ok
...
//Checkをlabelで囲む。―――――――――――――――――――――――――――――――――――
	var inputcheck = $('input[type="checkbox"]').not(':first').not('[name="chkPurpose[]"]')
	var inputcheck2 = $('input[type="checkbox"][name="chkPurpose[]"]')

	$('input[name="chkPurpose[]"]:first').closest('td').attr('id','purposeTdID')
	var htmlStr = $('#purposeTdID').text().split('用')
	htmlStr[0] = htmlStr[0]+='用';
	htmlStr[1] = htmlStr[1]+='用';
	htmlStr[2] = htmlStr[2]+='用';
	htmlStr[3] = htmlStr[3]+='用';
	var TempPurposeTdID = $('<div>')
	...
	for (var i=0,len=inputcheck2.length; i<len; i++) {
		inputcheck2.eq(i).attr('id','chkPurposeInput_'+(i-0+1))//2つあるIDを一つにする
		var radiotxt = $('<span>').attr('id','chkPurposeSpan_'+(i-0+1)).html(htmlStr.shift())
		var radiolabelNode = $('<label>').addClass('labelClass').append(inputcheck2.eq(i).clone()).append(radiotxt)
		TempPurposeTdID.append(radiolabelNode)
	}

	$('#purposeTdID').html(TempPurposeTdID.html())
...
//onアトリビュートの削除
	...
	$('#txtHopeArea').css('ime-mode','active')
	...
	$('#privacyID').html($('<label>').addClass('labelClass').attr('for','chkAcceptProcess').append(privacyTdClone))
