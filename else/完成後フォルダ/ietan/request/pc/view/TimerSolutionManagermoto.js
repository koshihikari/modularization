...
jQuery(document).ready(function($){
	...
	MYNAMESPACE.view.TimerSolutionManager.prototype = {
		...
		,timerSolutionKanaAndPlaceholder: function() {//カナ自動入力に伴うremoveClass(blank)とforIE文字黒、PS消去
			$('form.NT_mainForm').on('blur', '.firstName', function() {
			...
			$('form.NT_mainForm').on('blur', '.lastName', function() {
			...
			if(($('html').hasClass('ie') === true)||($('html').hasClass('ie9') === true)){
				...
				$('form.NT_mainForm').on('keyup blur', '.firstName', function() {
				...
				$('form.NT_mainForm').on('keyup blur', '.lastName', function() {
				...
				$('form.NT_mainForm').on('focus', '.lastName', function() {
				...
				$('form.NT_mainForm').on('focus', '.firstName', function() {
