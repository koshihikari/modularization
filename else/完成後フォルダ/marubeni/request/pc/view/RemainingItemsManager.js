	...
	MYNAMESPACE.view.RemainingItemsManager.prototype = {
		...
		,insertdiv: function() {
			...
			var countspantext2 = $('<span>').addClass('countspantext2')
									...
									.append($('<span>').addClass('countspanNum').text($('.NT_blank').length))
									// .append($('<span>').addClass('countspanNum').text($('.NT_blank').not('#MtxtEMAIL1,#MtxtEMAIL2').length))
			...
			floatDiv.hide();
			$('form.NT_mainForm').after(floatDiv);


			if($('.NT_blank').length === 0){//未入力なし
				countli.hide();
		...
		,remainingItemsCheck: function() {
			var thisObj = this;
			setTimeout(function(){
				if($('.NT_blank').length !== 0){
					$('#float').removeClass('countOk')
					$('li.countOkli').hide();
					$('li.countli').show('slow');
					$('span.countspanNum').text($('.NT_blank').length)
//丸紅追加―――――――――――――――――――――――――
					$('#confirm_button').hide()
					$('#confirm_button2').fadeIn('slow')
//―――――――――――――――――――――――――
				} else {
