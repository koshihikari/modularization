...
jQuery(document).ready(function($){
	...
	MYNAMESPACE.view.SubmitCheckManager.prototype = {
		...
		,submitCheck: function() {
			var thisObj = this;
			$j('form.NT_mainForm').submit(function(){
			// $j("form.NT_mainForm").submit(function() {//細かく対応。
						...
						var isCorrect = true;
						if($j('.NT_blank').length == 0){//submit エラーなし対応
...
// alert('チェック用アラート：true end')
							...
							// alert('送信（サンプルフォームのため、送信されません。）')
							// $("form.NT_mainForm").submit()
...
// //for yuraku　↓―――――――――――――――――――――――――――――――――――
// 							var FirstBlank = $j('form.NT_mainForm .NT_blank').eq(0)
...
// //for yuraku　↑―――――――――――――――――――――――――――――――――――
							if($('html').hasClass('ie') === true){//placeholderの値削除(IE)
								var targetY = $j('form.NT_mainForm .NT_blank').eq(0).offset().top - 100;
								$j('body,html').animate({scrollTop: targetY}, 0);
								$j('form.NT_mainForm .NT_blank').eq(0).blur();
								$('input:not(input[type="radio"])').blur()
							} else {
								var targetY = $('form.NT_mainForm .NT_blank').eq(0).offset().top - 100;
								$j('form.NT_mainForm .NT_blank').eq(0).blur();
